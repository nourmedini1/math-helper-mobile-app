...
class _SumPageState extends State<SumPage> with TickerProviderStateMixin {
        ...
        builder: (context, state) {
          if (state is SymbolicSumInitial) {
            ...
            return successWidget(
                context,
                "Symbolic Sum",
                state.response.summation,
                state.response.convergent
                    ? state.response.result.toString()
                    : r"\text{This sum series is divergent}",
                "symbolic");
          } else if (state is SymbolicSumFailure) {
            return symbolicInitialWidget(context);
          }
          return Container();
        },
          ...
          if (state is NumericSumInitial) {
            ...
            return successWidget(
                ...
                state.response.convergent
                    ...
                    : r"\text{This sum series is divergent}",
                "numeric");
          } else if (state is NumericSumFailure) {
            return numericInitialWidget(context);
          }
          return Container();
        },
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: textField(
                          context,
                          symbolicLowerLimitController,
                          " The lower limit of the sum series",
                          "symbolic",
                          double.infinity),
                    ),
                  ],
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Padding(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    sumResult(context, "The Sum result", expression, result),
                    const SizedBox(
                      height: 20,
                    )
                  ],
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 40),
            child: sumResetButton(context, operation),
          ),
        ],
      ),
    );
  ...
  Widget sumResetButton(BuildContext context, String operation) {
    return GestureDetector(
      onTap: () {
        if (operation == "symbolic") {
          BlocProvider.of<SymbolicSumBloc>(context)
              .add(const SymbolicSumReset());
        } else {
          BlocProvider.of<NumericSumBloc>(context).add(const NumericSumReset());
        }
      },
              ...
              ? () {
                  if (operation == 'symbolic') {
                    final request = SumRequest(
                        ...
                        upperLimit: "n");
                    BlocProvider.of<SymbolicSumBloc>(context).add(
                      SymbolicSumRequested(request: request),
                    );
                  } else {
                    final request = SumRequest(
                    ...
                    BlocProvider.of<NumericSumBloc>(context).add(
                      NumericSumRequested(request: request),
                    );
                  }
                }
              : () {},
