...
class FirstGraphSubmitButton extends StatelessWidget {
  final TextEditingController functionController;
  final bool isFirstPlot;

  const FirstGraphSubmitButton({super.key, required this.functionController, required this.isFirstPlot});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<FirstGraphFieldsCubit, FirstGraphFieldsState>(
      builder: (context, state) {
        final theme = Provider.of<ThemeManager>(context).themeData;
        final color = theme == AppThemeData.lightTheme
            ? AppColors.primaryColorTint50
            : AppColors.primaryColor;
        if (state is FirstGraphFieldsReady) {
          return SubmitButton(
