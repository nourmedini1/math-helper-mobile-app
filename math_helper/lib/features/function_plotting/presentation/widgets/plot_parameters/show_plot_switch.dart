...
class PlotShowPlotSwitch extends StatelessWidget {
  final bool isLight;
  final bool isFirstPlot;
  final GraphData graphData;

  const PlotShowPlotSwitch({
    super.key,
  ...
  Widget build(BuildContext context) {
    return SwitchListTile(
      title: const Padding(
        ...
        child: TextFieldLabel(label: "Show Plot"),
      ),
      value: graphData.isVisible,
      onChanged: (value) {
        if (isFirstPlot) {
          context.read<GraphCubit>().updateFirstGraph(isVisible: value);
        } else {
          context.read<GraphCubit>().updateSecondGraph(isVisible: value);
        }
      },
